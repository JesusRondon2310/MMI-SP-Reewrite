    ...
    public class InsuranceObserver : Script
    {
            ...
            if (_previousVehicle != Game.Player.Character.CurrentVehicle)
            {
                ...
                if (_previousVehicle != null)
                {
                    ...
                    if (InsuranceManager.IsVehicleInsurable(_previousVehicle))
                    {
                        ...
                        {
                            new Sprite("mmi-sp", "insured", new System.Drawing.Point((int)(GTA.UI.Screen.Width * 0.955f), (int)(GTA.UI.Screen.Height * 0.83f)), new System.Drawing.Size(45, 45), 0f, System.Drawing.Color.FromArgb(35, 199, 128)).Draw();
                        ...
                        {
                            new Sprite("mmi-sp", "insured", new System.Drawing.Point((int)(GTA.UI.Screen.Width * 0.955f), (int)(GTA.UI.Screen.Height * 0.83f)), new System.Drawing.Size(45, 45), 0f, System.Drawing.Color.FromArgb(190, 0, 50)).Draw();
        ...
        private void CheckForInsuredVehicles()
        {
            ...
            foreach (Vehicle veh in array)
            {
                if (!veh.IsDead)
                {
                    ...
                    {
                        if (veh.Mods.LicensePlate == "46EEK572") veh.Mods.LicensePlate = "MMI" + new Random().Next(100, 999);
            ...
            for (int i = InsuredVehList.Count - 1; i >= 0; i--)
            {
                ...
                if (currenVeh.Exists())
                {
                    if (currenVeh.IsDead)
                    {
                        ...
                        Utils.ShowNotification("char_mp_mors_mutual", "MORS MUTUAL INSURANCE", T.GetString("NotifyVehicleDestroyedTitle"), T.GetString("NotifyVehicleDestroyedSubtitle"));
                ...
                if (Game.Player.LastVehicle == recoveredVehicle || !recoveredVehicle.Exists() || recoveredVehicle.IsDead)
                {
                    if (recoveredVehicle.Exists())
                    {
                        if (recoveredVehicle.IsAlive)
                            Utils.ShowNotification("char_mp_mors_mutual", "MORS MUTUAL INSURANCE", T.GetString("NotifyVehicleRecoveredTitle"), T.GetString("NotifyVehicleRecoveredSubtitle"));
        ...
        private void UpdateIncomingVehicles()
        {
            for (int i = IncomingVehicles.Count - 1; i >= 0; i--)
            {
                ...
                if (incoming.vehicle.AttachedBlip.Sprite == BlipSprite.ArmsTraffickingAir ||
                    incoming.vehicle.AttachedBlip.Sprite == BlipSprite.Tank ||
                    incoming.vehicle.AttachedBlip.Sprite == BlipSprite.Speedboat ||
                    incoming.vehicle.AttachedBlip.Sprite == BlipSprite.GunCar)
                ...
                if (!incoming.driver.IsInVehicle(incoming.vehicle))
                {
                    ...
                    if (incoming.driver.Position.DistanceTo(Game.Player.Character.Position) < 8.0f)
                    {
                        ...
                        // Usamos el método nativo de la clase Ped para evitar el FATAL ERROR del hash
                        incoming.driver.PlayAmbientSpeech(speech.Name, speech.Voice);
                    }
        ...
        {
            Utils.ShowNotification("CHAR_MP_MORS_MUTUAL", "MORS MUTUAL INSURANCE", T.GetString("BringVehicle"), T.GetString("NotifyBringVehicleCancel"));

            // Refund the player
            if (refund == 0)
                Game.Player.Money += incoming.price;
            else
                Game.Player.Money += (refund + incoming.price);
