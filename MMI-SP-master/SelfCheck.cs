...
namespace MMI_SP
{
    internal static class SelfCheck
    {
        ...
        {
            new Dependency("ScriptHookVDotNet3.dll", "3.6.0.0", AppDomain.CurrentDomain.BaseDirectory + @"\.."),
            new Dependency("iFruitAddon2.dll", "3.1.1.0"),
        ...
        private static bool CheckDependencies()
        {
            ...
            foreach (Dependency dependency in Dependencies)
            {
                ...
                {
                    // Obtenemos la versión como texto y limpiamos cualquier carácter extraño
                    string rawVersion = FileVersionInfo.GetVersionInfo(dependency.FullPath).ProductVersion;
                    Version installedVersion;

                    // Intentamos leer la versión, si falla (como con el build 3725), usamos una por defecto
                    if (!Version.TryParse(rawVersion.Split(' ')[0], out installedVersion))
                    {
