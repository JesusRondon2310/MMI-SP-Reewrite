...
namespace MMI_SP
{
    static class Price
    {
        ...
        public static int GetVehicleSizePrice(Vehicle veh)
        {
            ...
            // Obtener dimensiones correctamente en SHVDN v3
            Vector3 min, max;
            veh.Model.GetDimensions(out min, out max);

            // Usamos 'dimensions' como nombre de variable para no romper los 'if' de abajo
            Vector3 dimensions = max - min;

            if (veh.Model.IsPlane)
            {
                // Vehicle Length (Eje Y)
                if (dimensions.Y < 10f)
                    value += 1000;
            ...
            else if (veh.ClassType == VehicleClass.Cycles) // Los Quads suelen entrar aquí o en OffRoad
            {
                return 0;
            }
        ...
        public static int GetVehicleModsPrice(Vehicle veh)
        {
            ...
            // Mods normales (Index > -1 significa que tiene algo instalado)
            ...
            // Colores y propiedades de estado
            if (veh.Mods.IsPrimaryColorCustom)
                value += 700;
            if (veh.Mods.IsSecondaryColorCustom)
                value += 500;

            if (veh.IsConvertible) // Esta propiedad se mantiene igual
                value += 525;

            if (!veh.CanTiresBurst) // Esta propiedad se mantiene igual
                value += 810;
